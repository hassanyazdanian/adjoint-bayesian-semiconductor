...
def sigmoid_np(x):
    """
    Stable enough sigmoid for post-processing raw plateau samples.
    """
    x = np.asarray(x, dtype=float)
    return 1.0 / (1.0 + np.exp(-x))


def bounded_np(raw, low, high):
    """
    Map raw samples to the bounded interval [low, high].
    """
    return float(low) + (float(high) - float(low)) * sigmoid_np(raw)
...
def get_obs_plateaus(obs):
    """
    Return true/default p-type and n-type plateau values.
    """
    return float(obs["C_p"]), float(obs["C_n"])


def set_prior_plateaus(prior, cp, cn):
    """
    Set plateau values in TorchMaternCov
    """
    cp_t = torch.tensor(float(cp), dtype=torch.float64, device=prior.device)
    cn_t = torch.tensor(float(cn), dtype=torch.float64, device=prior.device)
    prior.C_p = cp_t
    prior.C_n = cn_t
...
def extract_samples(stat, obs):
    """
    Extract p, cp, cn samples from pCN/pCN+Gibbs output.
    """
...
def build_prior(obs, pde, stat):
    ...
    cp_true, cn_true = get_obs_plateaus(obs)
...
def postprocess_pcn(
    sample_file="./stat/pcn_known_cp_cn.pickle",
    data_file="./obs/obs_synth_top.pickle",
    outdir="./stat_postprocess_pcn",
    cred_mass=0.95,
    x_probe=0.5,
    ...
    if burn < 0 or burn >= S.shape[0]:
        raise ValueError(f"burn must be in [0, {S.shape[0]-1}].")
    ...
    if burn > 0 or thin > 1:
        S = S[burn::thin, :]
        cp_samples = cp_samples[burn::thin]
        cn_samples = cn_samples[burn::thin]

        if cp_raw is not None:
            cp_raw = cp_raw[burn::thin]
        if cn_raw is not None:
            cn_raw = cn_raw[burn::thin]

        info["burn_applied_in_postprocess"] = burn
        info["thin_applied_in_postprocess"] = thin
        info["n_total_after_burn_thin"] = S.shape[0]
    ...
    # --------------------------------------------------------
    # Choose field samples
    # --------------------------------------------------------
    if max_field_samples is None:
        field_indices = np.arange(total_samples, dtype=int)
    else:
        max_field_samples = int(max_field_samples)
        if max_field_samples < 1:
            raise ValueError("max_field_samples must be None or a positive integer.")

        n_use = min(max_field_samples, total_samples)
        field_indices = np.unique(
            np.linspace(0, total_samples - 1, n_use, dtype=int)
        )
    ...
    with torch.no_grad():
        for kk, ii in enumerate(field_indices):
            ...
            set_prior_plateaus(prior, cp_i, cn_i)
    ...
    np.save(os.path.join(outdir, "field_indices.npy"), field_indices)
    ...
    # Save text summary
    # --------------------------------------------------------
    summary_path = os.path.join(outdir, "pcn_clean_summary.txt")

    with open(summary_path, "w") as f:
        f.write("pCN clean post-processing summary\n")
        f.write("=================================\n\n")
...
if __name__ == "__main__":
    ...
    postprocess_pcn(
        sample_file=f"./stat_Setting{Setting}/pcn_Setting{Setting}_Ex{Example}.pickle",
        data_file=f"./obs/obs_synth_top_Ex{Example}.pickle",
        outdir=f"./postprocess/pcn_Setting{Setting}_Ex{Example}",
        cred_mass=0.95,
        x_probe=0.5,
        use_mean_depth=False,
